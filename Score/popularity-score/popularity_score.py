...
# 定义改进版的知名度评分规则函数
def calculate_improved_popularity_score(stars, forks, a=1, b=0.5, c=0.05):
...
try:
    ...
    # 确保数据中包含所需的列
    if all(col in repo_df.columns for col in ["stargazers_count", "forks_count", "repo_name", "platform"]):
        # 计算改进后的知名度得分
        ...
        result_df = repo_df[["id", "platform", "repo_name",  "popularity_score"]]

        # 保存带有知名度得分的新文件
        result_df.to_csv(output_file, index=False)
        print(f"计算完成，结果已保存到 {output_file}")
    else:
        print("数据中缺少所需列（stargazers_count 或 forks_count 或 repo_name 或 platform）。")

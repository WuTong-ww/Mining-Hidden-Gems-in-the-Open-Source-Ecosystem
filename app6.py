...
def generate_analysis():
    ...
    if repo_readme:
        prompt = f"""
            以下是一个开源项目的 README 内容，以及该项目的评分信息（创新性、复杂度、知名度）。请根据这些信息自动生成冷门原因分析，并给出相应的优化建议：\n\n
            项目 README：\n{repo_readme}\n\n
            项目评分：\n
            创新性评分：{innovation}\n
            复杂度评分：{complexity}\n
        ...
        prompt = f"""
            以下是一个开源项目的评分信息（创新性、复杂度、知名度）。请根据这些信息自动生成冷门原因分析，并给出相应的优化建议：\n\n
            项目评分：\n
            创新性评分：{innovation}\n
            复杂度评分：{complexity}\n
            知名度评分：{popularity}\n\n
            ...
            """
    try:
        generator = pipeline("text-generation", model="D:/Learning/MyCode/intro/model", truncation=True)
        response = generator(prompt, max_length=600, num_return_sequences=1, do_sample=True, top_k=50, top_p=0.95)
